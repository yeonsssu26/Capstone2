...
class ChargingStation(gym.Env):
    metadata = {'render.modes': ['human', 'rgb_array']}

    def __init__(self, grid_shape=(10, 10), n_agents=3, n_preys=8, prey_move_probs=(0.175, 0.175, 0.175, 0.175, 0.3),
                full_observable=False, penalty=-0.5, step_cost=-0.01, prey_capture_reward=20, max_steps=10000000,
                agent_view_mask=(10, 10)):
        ...
        self.action_space = MultiAgentActionSpace([spaces.Discrete(5) for _ in range(self.n_agents)]) #  Discrete(5) has 5 possible values in this space.
        ...
        # auction theory
        self.agent_avail_charging = None    
        self.prey_need_charging = None      
        self.price = None                
    ...
    def reset(self):
        ...
        # auction theory
        self.price = [random.randint(5,20) for _ in range(self.n_agents)] # agent가 팔 price (prey가 agent를 선택하는 구조로 적용)
        self.agent_avail_charging = [random.randint(1000, 10000) for _ in range(self.n_agents)]
        self.prey_need_charging = [random.randint(10, 100) for _ in range(self.n_preys)]
    ...
    def __update_prey_pos(self, prey_i, move):
        ...
        if self._prey_alive[prey_i]:
            ...
            self._full_obs[curr_pos[0]][curr_pos[1]] = PRE_IDS['empty'] # 잡아먹힘
    ...
    def step(self, agents_action):
        ...
        for prey_i in range(self.n_preys):
            if self._prey_alive[prey_i]:
                predator_neighbour_count, n_i = self._neighbour_agents(self.prey_pos[prey_i]) # 먹혔는지 아닌지

                #############################################################################################
                # auction theory
                # prey 주변에 predator이 2개 이상이기 때문에 bidding
